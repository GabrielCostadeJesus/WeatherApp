    ...
    public WeatherHourForecast getHourForecastWeather(String data) throws WeatherLibException {
        ...
        try {
            ...
            if (jObj.has("city")) {
                ...
                loc.setCity(cityObj.getString("name"));

                if (cityObj.has("coord")) {
                    JSONObject cooObj = cityObj.getJSONObject("coord");
                    ...
                    if (cooObj.has("lon")) {
                        loc.setLatitude((float) cooObj.getDouble("lon"));
                    }
                }

                if (cityObj.has("country")) {
                    loc.setCountry(cityObj.getString("country"));
                }
                if (cityObj.has("population")) {
                    loc.setPopulation(cityObj.getLong("population"));
                }
            }
            ...
            for (int i = 0; i < wList.length(); i++) {
                ...
                hourForecast.weather.location = loc;
